 ...
 * @author a_escobar
 */
public class LlistarProcessionariaController extends AbstractController {

    private IAfectacioService afectacioService;

    public LlistarProcessionariaController(){
        super();
    }
    ...
    protected ModelAndView handleRequestInternal(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        for(int i=0;i<usuari.getAuthorities().size();i++){
            editable = editable || "ROLE_ESBORRAR_PROC".equals(usuari.getAuthorities().get(i).getAuthority());
