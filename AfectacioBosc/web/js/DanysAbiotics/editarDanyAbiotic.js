...
YAHOO.util.Event.onContentReady("dadesestimacions",function() {
    ...
    YAHOO.example.InlineCellEditing = function() {
        ...
        var myColumnDefs = [
            ...
            {key:"estrat", label:txtEstrat, editor: new YAHOO.widget.DropdownCellEditor({dropdownOptions:["VOL","SUBVOL"],disableBtns:true})},
        ...
        myDataSource.responseSchema = {
            fields: ["id","ordre","idespecie","especie","recobriment","afectacio","estrat",{key:"id"}]
        };
...
var onButtonAfegirAfectacioEstimadaClick = function onButtonClick(p_oEvent) {
    var posicio = myDataTableEstimacions.getRecordSet().getLength();
    myDataTableEstimacions.addRow( {id:"-1", especie:"",recobriment:"",afectacio:"",estrat:"",ordre:posicio+1} , posicio);
};

function getDadesEstimacions(){
    var set = myDataTableEstimacions.getRecordSet();
    var jsonStr = YAHOO.lang.JSON.stringify(set);
    document.formAfectacio.elements['idsDadesEstimacions'].value = jsonStr;
}
...
function comprovarFilaTaulaEstimacions(record){
    var dada = record.getData('especie');
    if(dada==null || trim(dada)==''){
        return false;
    }
    dada = record.getData('recobriment');
    dada = dada.replace(",",".");
